...
def multiperiod_simulation(weights, expected_returns, volatilities, correlations, horizon, rebalance_freq, num_simulations):
    ...
    total_periods = horizon * periods_per_year
    # Scale for period: assume annual inputs, so per period mean = expected / periods_per_year, vol = vol / sqrt(periods_per_year)
    ...
    for _ in range(num_simulations):
        asset_values = np.array(weights)  # Start with $1 allocated
        for p in range(total_periods):
            ...
            if (p + 1) % periods_per_year == 0:  # Rebalance at freq

...
def optimize_portfolio(method, expected_returns, volatilities, correlations, dividend_yields, inflation, tax_rate, risk_free_rate, iterations, use_sharpe, use_inflation, use_tax_rate, use_advanced_metrics, min_weight=None, max_weight=None):
    ...
    min_weights = [min_weight] * num_assets if min_weight is not None else None
    max_weights = [max_weight] * num_assets if max_weight is not None else None
    
    if method == "Monte Carlo":
        weights = monte_carlo_optimize(expected_returns, cov_matrix, iterations, num_assets, risk_free_rate, min_weights, max_weights)
    elif method == "Genetic Algorithm":
        weights = genetic_algorithm_optimize(expected_returns, cov_matrix, iterations, num_assets, risk_free_rate, min_weights, max_weights)
    elif method == "Gradient Descent (Mean-Variance)":
        weights = gradient_descent_optimize(expected_returns, cov_matrix, num_assets, risk_free_rate, min_weights, max_weights)
    elif method == "SciPy (Constrained)":
        weights = scipy_optimize(expected_returns, cov_matrix, num_assets, risk_free_rate, min_weights, max_weights)
    else:
...
def calculate_metrics(weights, expected_returns, cov_matrix, dividend_yields, inflation, tax_rate, risk_free_rate, use_sharpe, use_inflation, use_tax_rate, use_advanced_metrics):
    metrics = {}
    ...
    if use_advanced_metrics:
        # Assume normal distribution for VaR 5%
        metrics['VaR 5%'] = portfolio_return - 1.645 * portfolio_volatility
        # Approximate Sortino (target=0, normal dist approx downside vol = vol / sqrt(2))
...
def monte_carlo_optimize(returns, cov, iterations, num_assets, rf_rate, min_weights, max_weights):
    best_sharpe = -np.inf
    best_weights = None
...
def genetic_algorithm_optimize(returns, cov, iterations, num_assets, rf_rate, min_weights, max_weights):
    population_size = 50
...
def gradient_descent_optimize(returns, cov, num_assets, rf_rate, min_weights, max_weights, max_iter=1000, lr=0.01):
...
def scipy_optimize(returns, cov, num_assets, rf_rate, min_weights, max_weights):
    def objective(w):
        port_ret = np.dot(w, returns)
        port_vol = np.sqrt(np.dot(w.T, np.dot(cov, w)) + 1e-10)
        return - (port_ret - rf_rate) / port_vol  # Maximize Sharpe by minimizing negative
    
    constraints = ({'type': 'eq', 'fun': lambda w: np.sum(w) - 1})
    bounds = list(zip(min_weights or [0]*num_assets, max_weights or [1]*num_assets))
